...
export default function Bookings() {
  ...
  // State to trigger data refreshes after adding a new booking
    ...
    const fetchTimelineData = async () => {
      ...
      try {
        ...
        if (storeHourRes.ok) {
          ...
          isClosed =
            storeHourData?.formattedGetEffectiveStoreHour?.isClosed || false;
          ...
          if (!isClosed) {
            ...
            if (openCloseTimes.length > 0) {
              newStart = openCloseTimes.reduce(
                (min: string, t: any) => (t.openTime < min ? t.openTime : min),
                openCloseTimes[0].openTime,
              );
              newEnd = openCloseTimes.reduce(
                (max: string, t: any) =>
                  t.closeTime > max ? t.closeTime : max,
                openCloseTimes[0].closeTime,
              );
            }
          }
        }
        ...
        try {
          const exceptionsPromises = mappedTables.map(
            (t) =>
              fetch(
                `${baseUrl}/restaurant/${restaurantId}/table/${t.id}/exception?date=${queryDate}`,
              )
  ...
  const bookingsByTable = useMemo(() => {
    ...
    for (const [k, v] of map.entries()) {
      v.sort(
        (a, c) =>
          parseTimeToMinutes(a.startTime) - parseTimeToMinutes(c.startTime),
      );
      map.set(k, v);
    }
    return map;
  ...
  const stretchedSlotWidth =
    numSlots > 0 ? availableTimelineWidth / numSlots : minSlotWidthPx;
  ...
  const stretchedRowHeight =
    tables.length > 0
      ? availableTimelineHeight / tables.length
      : minRowHeightPx;
    ...
    <div className="flex flex-col p-8 h-[calc(100vh-70px)] overflow-hidden">
      ...
      >
        ...
        {!isLoading && storeHours.isClosed && (
          <div className="absolute inset-0 bg-gray-50 z-40 flex items-center justify-center flex-col gap-2">
            ...
            <span className="text-sm text-gray-400">
              There are no operating hours for this date.
            </span>
          </div>
        )}
        ...
        <div className="overflow-x-auto overflow-y-auto flex-1 min-h-0">
          <div
            style={{
              width: timeColWidthPx + timelineWidthPx,
              minWidth: "100%",
            }}
          >
            {/* Header row */}
            <div
              className="flex sticky top-0 z-30 bg-gray-50 border-b border-gray-200 shadow-sm"
            ...
            {tables.map((table) => {
              ...
              const thisTableExceptions = tableExceptions.filter(
                  ...
                  >
                    ...
                    <div className="absolute inset-0 flex pointer-events-none">
                      {Array.from({ length: numSlots }).map((_, slotIdx) => (
                        <div
                          key={slotIdx}
                          className="border-r border-gray-100 flex-shrink-0 h-full"
                          style={{ width: slotWidthPx }}
                        />
                      ))}
                    </div>
