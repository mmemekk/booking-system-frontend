...
export default function Dashboard() {
  ...
  const [metric2Data, setMetric2Data] = useState({
    bookedSlots: 0,
    totalStoreSlots: 0,
    percentage: 0,
  });
    ...
    const fetchBookings = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${baseUrl}/booking/${restaurantId}?bookingDate=${queryDate}`,
        );
        ...
        const mappedBookings: Booking[] = data.formattedBooking.map(
          (b: any) => ({
            id: String(b.id),
    ...
    const fetchUtilization = async () => {
      try {
        ...
        const [storeHourRes, tablesRes, tableAvailRes] = await Promise.all([
          fetch(
            `${baseUrl}/restaurant/${restaurantId}/availability/store-hour/?date=${queryDate}`,
          ),
          fetch(`${baseUrl}/restaurant/${restaurantId}/table`),
          fetch(
            `${baseUrl}/restaurant/${restaurantId}/availability/table/?date=${queryDate}`,
          ),
        ]);
        ...
        if (storeHourRes.ok) {
          ...
          const isClosed =
            storeHourData?.formattedGetEffectiveStoreHour?.isClosed;

          if (!isClosed) {
            const openCloseTimes =
              storeHourData?.formattedGetEffectiveStoreHour?.openCloseTimes ||
              [];
            openCloseTimes.forEach((t: any) => {
              totalStoreMinutes +=
                timeToMinutes(t.closeTime) - timeToMinutes(t.openTime);
            });
          }
        }
        ...
        if (tableAvailRes.ok) {
          ...
          const availabilities =
            tableAvailData?.formattedGetEffectiveTableAvailability || [];

          availabilities.forEach((table: any) => {
            if (table.availabilities) {
              table.availabilities.forEach((chunk: any) => {
                totalFreeMinutes +=
                  timeToMinutes(chunk.closeTime) -
                  timeToMinutes(chunk.openTime);
              });
            }
          });
        }
        ...
        const totalStoreSlots = Math.floor(
          (totalStoreMinutes / slotDuration) * numTables,
        );
        ...
        const percentage =
          totalStoreSlots === 0
            ? 0
            : Math.round((bookedSlots / totalStoreSlots) * 100);
  ...
  const coverPercentage =
    activeTotalCovers === 0
      ? 0
      : Math.round((seatedCovers / activeTotalCovers) * 100);

  const radius = 36;
  const circumference = 2 * Math.PI * radius;
  const strokeDashoffsetMetric1 =
    circumference - (coverPercentage / 100) * circumference;

  // --- METRIC 2 CALCULATIONS (Waffle Grid) ---
  const activeSquares =
    metric2Data.percentage > 0
  ...
  const attritionRate =
    totalBookingsCount === 0
      ? 0
      : Math.round((lostBookingsCount / totalBookingsCount) * 100);

  // Percentages for the stacked horizontal bar
  const successPct =
    totalBookingsCount === 0 ? 0 : (successCount / totalBookingsCount) * 100;
  const createdPct =
    totalBookingsCount === 0 ? 0 : (createdCount / totalBookingsCount) * 100;
  const noShowPct =
    totalBookingsCount === 0 ? 0 : (noShowCount / totalBookingsCount) * 100;
  const canceledPct =
    totalBookingsCount === 0 ? 0 : (canceledCount / totalBookingsCount) * 100;
  ...
  const renderBookingCard = (booking: Booking, isUpcoming: boolean) => (
    ...
    >
      <div
        className={`flex flex-col items-center justify-center rounded-md bg-gray-100 p-3 transition-colors ${
          isUpcoming ? "group-hover:bg-blue-light" : "group-hover:bg-gray-100"
        }`}
      >
        <span
          className={`text-lg font-bold text-grey-heading ${
            isUpcoming
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex items-center gap-2">
          <p
            className={`font-semibold text-grey-heading leading-none ${
              isUpcoming
                ? "group-hover:text-blue-dark"
                : "group-hover:text-gray-900"
            }`}
      ...
      <div className="ml-auto mr-4 flex items-center gap-4">
        {isUpcoming ? (
          <>
            <button
              onClick={(e) =>
                handleUpdateStatus(e, booking.bookingRef, "success")
              }
            ...
            <div className="relative">
              ...
              {actionMenuId === booking.id && (
                <div className="absolute right-12 top-0 w-40 bg-white rounded-lg shadow-[0_4px_20px_rgba(0,0,0,0.15)] border border-gray-100 py-1.5 z-50">
                  <button
                    onClick={(e) =>
                      handleUpdateStatus(e, booking.bookingRef, "noshow")
                    }
                  ...
                  <button
                    onClick={(e) =>
                      handleUpdateStatus(e, booking.bookingRef, "canceled")
                    }
          ...
          <div className="flex flex-col items-end gap-2">
            <span
              className={`text-[12px] leading-none font-bold px-3 py-1.5 rounded-full capitalize ${
            ...
            <button
              onClick={(e) =>
                handleUpdateStatus(e, booking.bookingRef, "created")
              }
      ...
      <div className="flex gap-5 justify-between items-stretch shrink-0">
          ...
          <div className="relative flex items-center justify-center h-20 w-20">
            <svg className="transform -rotate-90 w-20 h-20">
              ...
              <circle
                ...
                strokeDashoffset={strokeDashoffsetMetric1}
                strokeLinecap="round"
                className={`transition-all duration-1000 ease-out ${coverPercentage === 100 ? "text-green-500" : "text-blue-500"}`}
              />
            </svg>
        ...
        <div className="flex bg-white border border-border rounded-lg p-6 w-full items-center justify-between shadow-sm">
          <div className="flex flex-col gap-3">
            ...
            <p className="text-sm text-gray-400 mt-1">
              {metric2Data.bookedSlots} / {metric2Data.totalStoreSlots} slots
              reserved
            </p>
          </div>
